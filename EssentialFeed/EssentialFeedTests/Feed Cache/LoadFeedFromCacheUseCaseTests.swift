...
class LoadFeedFromCacheUseCaseTests: XCTestCase {
    ...
    func test_load_deliversCacheImagesOnNonExpiredache() {
    ...
    func test_load_deliversNoImagesOnExpirationCache() {
        ...
        expect(sut, toCompleteWith: .success([])) {
            store.completeRetrievalSuccessful(with: feed.local, timestamp: expirationTimestamp)
        }
    }
    ...
    func test_load_hasNoSideEffecstsOnExpirationCache() {
        let fixedCurrentDate = Date()
        let expirationTimestamp = fixedCurrentDate.minusFeedCacheMaxAge()
        let (sut, store) = makeSUT { fixedCurrentDate }
